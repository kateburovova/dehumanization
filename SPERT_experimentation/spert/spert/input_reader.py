...
class BaseInputReader(ABC):
    def __init__(self, types_path: str, tokenizer: AutoTokenizer, neg_entity_count: int = None,
...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_document(self, doc, dataset) -> Document:
        ...
        jtokens = doc['tokens']
        # print('jtokens in document:', jtokens)
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset) -> List[Entity]:
        ...
        # print('doc_tokens supplied for parsing:', doc_tokens)

        for entity_idx, jentity in enumerate(jentities):
            # print('jentity:', jentity)
            ...
            # print("tokens in _parse_entities:", tokens)
            phrase = " ".join([t.phrase for t in tokens])
            entity = dataset.create_entity(entity_type, tokens, phrase)
            entities.append(entity)
        # print('Finished parsing entities ---------------------------------')
...
class JsonPredictionInputReader(BaseInputReader):
    def __init__(self, types_path: str, tokenizer: AutoTokenizer, spacy_model: str = None,
...
def _parse_tokens(jtokens, dataset, tokenizer):
    ...
    for i, token_phrase in enumerate(jtokens):
        # print("Token phrase:", token_phrase)
        token_encoding = tokenizer.encode(token_phrase, add_special_tokens=False)
        # print("Token encoding:", token_encoding)  # Debugging line
    ...
    # print('doc_encoding CONVERTED:', doc_encoding)
    # print('Done parsing tokens *******************')
